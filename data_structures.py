...
class Forex:
    ...
    def from_dividends_row(row):
        ...
        new_div = Forex(currency=row.currency, 
                        date=row.date, 
                        amount=gross_amount,
                        comment="Dividend Payment")
        new_tax = Forex(currency=row.currency,
                        date=row.date,
                        amount=tax_amount,
...
class FIFOForex(FIFOObject):
    ...
    def from_dividends_row(row):
        ...
        assert (0 <= tax_quantity <= gross_quantity), "Expected non-negative Tax Withholding!"
        ...
        new_forex = FIFOForex(currency=row.currency, 
    ...
    def from_share_sale(row):
        ...
        assert net_proceeds >= 0, "Expected non-negative net proceeds from sale of shares!"
        new_forex = FIFOForex(currency=row.currency,
...
class FIFOShare(FIFOObject):
    ...
    def from_deposits_row(row):
        ...
        # if quantity > 0, all assets share the same, i.e. just duplicate them
        new_asset = FIFOShare(symbol=row.symbol, 
...
class FIFOQueue:
    ...
    def push(self, asset):
        if self.is_empty():
            ...
            idx = 0
            while (idx < len(self.assets)) and (asset.buy_date > self.assets[idx].buy_date):
                idx += 1
    ...
    def pop(self, quantity):
        if quantity > self.total_quantity + 0.05: # accounting for some rounding errors
            raise ValueError("Cannot pop quantity larger than all quantities in FIFOQueue!")
...
def from_asset(asset, quantity):
    ...
    if asset.__class__.__name__ == "FIFOForex":
        ...
        new_asset = FIFOShare(asset.symbol, 
                              quantity, 
                              asset.buy_date, 
                              asset.buy_price, 
                              asset.currency)
        ...
        raise ValueError(f"asset is of unsupported type, got {asset_type}, expected 'FIFOForex' or 'FIFOShare'")
