...
class ReportData:
    ...
    def read_raw_data(self):
        self.daily_rates, self.monthly_rates, self.supported_currencies = get_reference_rates()

        self.df_deposits, self.df_sales, self.df_dividends, self.df_wire_transfers = read_data(self.sub_dir, self.file_name)
        ...
        if unsupported_currencies:
            raise ValueError(f"Currencies {unsupported_currencies} are not supported as exchange rate data is missing, check 'supported currencies' for automated reports.")
    ...
    def apply_exchange_rates(self):
        ...
        apply_rates_transact_dict(self.sold_shares, self.daily_rates, self.monthly_rates)
    ...
    def consolidate_report(self, report_year, mode):
        ...
        filtered_sold_forex = filter_transact_dict(self.sold_forex, report_year, 0.01, speculative_period=1)
    ...
    def create_excel_report(self, report_year, mode, report_file_name):
        df_shares, df_forex, df_dividends, df_fees, df_taxes = self.consolidate_report(report_year, mode)
        write_report(df_shares, df_forex, df_dividends, df_fees, df_taxes, self.sub_dir, report_file_name)
    ...
    def process_sales(self, df_sales):
        ...
        for row_idx, row in df_sales.iterrows():
            ...
            for t in tmp:
                ...
                assert row.currency == t.currency, f"Currency for buying share and selling share not the same, got {sold_share.currency} and {row.currency} respectively"
            ...
            new_fees = Forex(currency=row.currency, date=row.date, amount=row.fees, comment="Fees on Sale of Shares")
    ...
    def process_wire_transfers(self, df_wire_transfers):
        ...
        for row_idx, row in df_wire_transfers.iterrows():
            ...
            new_fees = Forex(currency=row.currency, date=row.date, amount=row.fees, comment="Fee on Wire Transfer")
            self.fees[row.currency].append(new_fees)
